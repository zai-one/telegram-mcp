...
def main():
    ...
    for key, default in list(values.items()):
        ...
        if default.endswith(".token"):
            ...
            entered = input(key + " (existing private env path; Enter for local login): ").strip()
            if not entered:
                import asyncio

                from login_telegram import create_session

                generated = secret_dir / "telegram.env"
                try:
                    asyncio.run(create_session(generated))
                except Exception:
                    raise SystemExit(
                        "Telegram login failed; inspect local setup and retry manually."
                    ) from None
                entered = str(generated)
